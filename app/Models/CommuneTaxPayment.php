...
/**
 * Suivi des paiements de taxes communales (ODP + TM) par commune et
 * par période. Voir migration pour la sémantique de period_type +
 * period_value.
 */
class CommuneTaxPayment extends Model
{
    ...
    /**
     * Statut du paiement vis-à-vis du théorique :
     *  - paye    : montant payé ≥ théorique
     *  - partiel : montant payé > 0 mais < théorique
     *  - non_paye: rien payé (ou enregistrement absent)
     */
    public function getStatusAttribute(): string
    {
        ...
        if ($paye >= $theorique - 1) return 'paye'; // tolérance arrondi
        return 'partiel';
    }

    public function getTotalTheoriqueAttribute(): float
    {
        return round((float) $this->odp_theorique + (float) $this->tm_theorique, 2);
    }

    public function getTotalPayeAttribute(): float
    {
        return round((float) $this->odp_paye + (float) $this->tm_paye, 2);
    }
