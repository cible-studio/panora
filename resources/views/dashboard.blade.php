<x-admin-layout>
    ...
    <x-slot name="topbarActions">
        ...
        <div style="position:relative;">
            <input type="text" id="search-input" placeholder="🔍 Rechercher campagne, client..."
        ...
        <div style="display:flex; gap:20px; align-items:flex-start;">
            ...
            <div style="flex:1; min-width:0;">
                ...
                <div class="stats-grid" style="grid-template-columns: repeat(3,1fr);">
                    ...
                    <a href="{{ route('admin.panels.index') }}" id="card-all" onclick="filterByStatus('all', this)"
                        style="text-decoration:none;" class="stat-card stat-card-clickable">
                    ...
                    {{-- Card 2 : Disponibles --}}
                    <a href="{{ route('admin.panels.index', ['status' => 'libre']) }}" id="card-libre"
                        style="text-decoration:none;" class="stat-card stat-card-clickable">
                    ...
                    {{-- Card 3 : CA Mensuel --}}
                    <a href="{{ route('admin.panels.index', ['status' => 'occupe']) }}" id="card-occupe"
                        style="text-decoration:none;" class="stat-card stat-card-clickable">
                        ...
                        <table>
                            ...
                            <tbody id="campaigns-tbody">
                                @forelse($campagnesRecentes as $campagne)
                                    <tr class="campaign-row" data-client="{{ strtolower($campagne->client->name) }}"
                                        ...
                                        <td style="width:100px;">
                                            <div style="background:var(--surface3); border-radius:4px; height:6px;">
                                                <div
                                                    style="background:var(--accent); height:6px; border-radius:4px; width:70%;">
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div style="display:flex;align-items:center;gap:8px;">
                            <span class="badge badge-green">Top 6</span>
                            <a href="{{ route('admin.rapports.index') }}" class="btn btn-ghost btn-sm">Voir tout</a>
                        </div>
                    </div>
                    <div class="card-body">
                            ...
                            @foreach ($tauxParCommune as $commune)
                                <div>
                                    <div style="display:flex; justify-content:space-between; margin-bottom:5px;">
                                        ...
                                        @if ($commune['taux'] >= 80)
                                            <span
                                                style="color:var(--accent); font-weight:700; font-size:13px;">{{ $commune['taux'] }}%</span>
                                        @elseif($commune['taux'] >= 60)
                                            <span
                                                style="color:var(--green); font-weight:700; font-size:13px;">{{ $commune['taux'] }}%</span>
                                        @else
                                            <span
                                                style="color:var(--blue); font-weight:700; font-size:13px;">{{ $commune['taux'] }}%</span>
                                        @endif
                                    </div>
                                    <div style="background:var(--surface3); border-radius:6px; height:20px;">
                                        @if ($commune['taux'] >= 80)
                                            <div
                                                style="background:var(--accent); height:20px; border-radius:6px; width:{{ $commune['taux'] }}%; display:flex; align-items:center; padding-left:8px;">
                                                <span
                                                    style="color:#000; font-size:11px; font-weight:700;">{{ $commune['taux'] }}%</span>
                                            </div>
                                        @elseif($commune['taux'] >= 60)
                                            <div
                                                style="background:var(--green); height:20px; border-radius:6px; width:{{ $commune['taux'] }}%; display:flex; align-items:center; padding-left:8px;">
                                                <span
                                                    style="color:#000; font-size:11px; font-weight:700;">{{ $commune['taux'] }}%</span>
                                            </div>
                                        @else
                                            <div
                                                style="background:var(--blue); height:20px; border-radius:6px; width:{{ $commune['taux'] }}%; display:flex; align-items:center; padding-left:8px;">
                                                <span
                                                    style="color:#fff; font-size:11px; font-weight:700;">{{ $commune['taux'] }}%</span>
                                            </div>
                                        @endif
                                    </div>
                                </div>
            ...
            <div style="width:270px; flex-shrink:0; display:flex; flex-direction:column; gap:16px;">
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div style="display:flex;align-items:center;gap:8px;">
                            <span class="badge badge-orange">{{ $reservationsEnAttente }}</span>
                            <a href="{{ route('admin.reservations.index') }}" class="btn btn-ghost btn-sm">Voir
                                tout</a>
                        </div>
                    </div>
                    <div class="card-body" style="padding:8px;">
                        @forelse($dernieresReservations->take(3) as $reservation)
                            <a href="{{ route('admin.reservations.show', $reservation) }}"
                                style="text-decoration:none;display:block;padding:8px;border-radius:8px;
                      margin-bottom:4px;transition:background .15s;"
                                onmouseover="this.style.background='var(--surface2)'"
                                onmouseout="this.style.background=''">
                                <div style="font-weight:600;font-size:12px;color:var(--text);">
                                    {{ $reservation->client->name }}</div>
                                <div style="color:var(--text3);font-size:11px;margin-top:2px;">
                                    {{ $reservation->panels->count() }} panneaux · {{ $reservation->reference }}
                                </div>
                            </a>
                        @empty
                            <div style="color:var(--text3);font-size:12px;text-align:center;padding:8px 0;">Aucune en
                                attente</div>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body" style="padding:8px;">
                        <div style="display:flex;flex-direction:column;gap:4px;">
                            @forelse($dernieresAlertes->take(3) as $alerte)
                                @php
                                    $color = match ($alerte->type) {
                                        'maintenance' => 'var(--red)',
                                ...
                                @endphp
                                <div
                                    style="font-size:12px;padding:7px 10px;border-radius:6px;border-left:3px solid {{ $color }};background:var(--surface2);color:{{ $color }};overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
                                    {{ $alerte->title }}
                                </div>
                            @empty
                                <div style="color:var(--text3);font-size:12px;text-align:center;padding:8px 0;">Aucune
                                    alerte</div>
                            @endforelse
                        </div>
                    </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body" style="padding:8px;">
                        @forelse($dernieresMaintenances->take(3) as $maintenance)
                            <a href="{{ route('admin.maintenances.show', $maintenance) }}"
                                style="text-decoration:none;display:block;padding:8px;border-radius:8px;
                      margin-bottom:4px;transition:background .15s;"
                                onmouseover="this.style.background='var(--surface2)'"
                                onmouseout="this.style.background=''">
                                <div style="font-weight:600;font-size:12px;color:var(--accent);">
                                    {{ $maintenance->panel->reference }}</div>
                                <div style="color:var(--text3);font-size:11px;margin-top:2px;">
                                    {{ Str::limit($maintenance->type_panne, 30) }}</div>
