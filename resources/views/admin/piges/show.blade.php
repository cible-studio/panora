        ...
        <div style="background:var(--surface);border:1px solid var(--border);border-radius:14px;overflow:hidden">
            ...
            <div style="padding:10px 16px;display:flex;gap:8px;align-items:center;border-top:1px solid var(--border)">
                <a href="{{ $pige->getPhotoUrl() }}"
                   ...
                   data-tech="{{ $pige->technicien?->name ?? '—' }}"
                   data-campaign="{{ $pige->campaign?->name ?? '' }}"
                   style="font-size:12px;color:var(--accent);text-decoration:none;display:flex;align-items:center;gap:5px;padding:6px 12px;background:rgba(232,160,32,.08);border:1px solid rgba(232,160,32,.25);border-radius:8px;cursor:pointer">
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 21l-4.35-4.35M11 4a7 7 0 1 1 0 14 7 7 0 0 1 0-14zM11 8v6M8 11h6"/></svg>
                    Agrandir l'image
                </a>
                @if($pige->hasGps())
                <a href="{{ $pige->getGoogleMapsUrl() }}" target="_blank"
