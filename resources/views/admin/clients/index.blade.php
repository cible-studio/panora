...
<div class="ci-stats-grid">
    ...
    <div class="ci-stat ci-stat-actions">
        ...
        <div style="display:flex;gap:8px">
            <button class="ci-export-btn" onclick="CI.toast('Export Excel en développement','info')">📊 Excel</button>
...
<div class="ci-filters">
    <div class="ci-filter-group">
        ...
        <div class="ci-input-icon">
            <svg width="15" height="15" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
            <input type="text" id="filter-search" class="ci-input" placeholder="Nom, NCC, email, téléphone…" autocomplete="off">
        </div>
    </div>
    <div class="ci-filter-group">
...
<style>
/* ── STATS ── */
.ci-stats-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px}
.ci-stat{background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:18px 16px;display:flex;align-items:center;gap:12px;transition:border-color .2s}
.ci-stat:hover{border-color:var(--accent)}
.ci-stat-gold{border-color:rgba(232,160,32,.2);background:rgba(232,160,32,.03)}
.ci-stat-actions{flex-direction:column;align-items:flex-start;gap:4px}
.ci-stat-icon{font-size:22px;flex-shrink:0}
.ci-stat-num{font-size:22px;font-weight:800;color:var(--text);line-height:1;font-family:var(--font-mono,monospace)}
.ci-stat-label{font-size:11px;color:var(--text3);margin-top:3px;font-weight:600;text-transform:uppercase;letter-spacing:.4px}
.ci-export-btn{background:var(--surface2);border:1px solid var(--border);color:var(--text2);border-radius:8px;padding:6px 12px;font-size:12px;cursor:pointer;transition:all .15s}
.ci-export-btn:hover{border-color:var(--accent);color:var(--accent)}

/* ── FILTRES ── */
.ci-filters{background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:16px 20px;margin-bottom:16px;display:flex;gap:12px;flex-wrap:wrap;align-items:flex-end}
.ci-filter-group{display:flex;flex-direction:column;gap:5px;flex:1;min-width:150px}
.ci-filter-label{font-size:11px;font-weight:600;color:var(--text3);text-transform:uppercase;letter-spacing:.5px}
.ci-input-icon{position:relative}
.ci-input-icon svg{position:absolute;left:10px;top:50%;transform:translateY(-50%);color:var(--text3);pointer-events:none}
.ci-input-icon .ci-input{padding-left:32px}
.ci-input,.ci-select{width:100%;background:var(--surface2);border:1px solid var(--border);color:var(--text);border-radius:8px;padding:9px 12px;font-size:13px;font-family:inherit;transition:border-color .15s}
.ci-input:focus,.ci-select:focus{outline:none;border-color:var(--accent)}
.ci-btn-reset{background:transparent;border:1px solid var(--border);color:var(--text2);border-radius:8px;padding:9px 14px;font-size:13px;cursor:pointer;transition:all .15s;width:100%}
.ci-btn-reset:hover{border-color:var(--accent);color:var(--accent)}

/* ── CARD / TABLE ── */
.ci-card{background:var(--surface);border:1px solid var(--border);border-radius:14px;overflow:hidden}
.ci-card-header{padding:14px 20px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between}
.ci-card-title{font-weight:700;font-size:14px}
.ci-count-badge{font-size:12px;color:var(--text3);background:var(--surface2);border:1px solid var(--border);border-radius:20px;padding:3px 12px}
.ci-table-wrap{overflow-x:auto;-webkit-overflow-scrolling:touch}
.ci-table{width:100%;border-collapse:collapse;min-width:600px}
.ci-table thead th{padding:11px 16px;text-align:left;font-size:11px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;border-bottom:1px solid var(--border);white-space:nowrap}
.ci-table tbody tr{border-bottom:1px solid rgba(255,255,255,.04);transition:background .12s}
.ci-table tbody tr:hover{background:rgba(255,255,255,.02)}
.ci-table tbody tr:last-child{border-bottom:none}
.ci-table td{padding:12px 16px;font-size:13px;color:var(--text2);vertical-align:middle}
.ci-pagination{padding:14px 16px;border-top:1px solid var(--border)}

/* ── SKELETON ── */
.ci-skel{background:linear-gradient(90deg,var(--surface2) 25%,var(--surface3,#1f2840) 50%,var(--surface2) 75%);background-size:200% 100%;animation:ci-skel 1.4s infinite}
@keyframes ci-skel{0%{background-position:200% 0}100%{background-position:-200% 0}}
...
.ci-modal{background:var(--surface);border:1px solid var(--border);border-radius:20px;padding:32px 28px;width:100%;max-width:420px;position:relative;animation:ci-modal-in .2s ease}
@keyframes ci-modal-in{from{opacity:0;transform:scale(.95) translateY(8px)}to{opacity:1;transform:scale(1) translateY(0)}}
.ci-modal-close{position:absolute;top:14px;right:14px;background:transparent;border:none;color:var(--text3);cursor:pointer;font-size:18px;width:28px;height:28px;display:flex;align-items:center;justify-content:center;border-radius:6px;transition:all .15s}
.ci-modal-close:hover{background:var(--surface2);color:var(--text)}
...
.ci-modal-icon-info{background:rgba(59,130,246,.1);border:1px solid rgba(59,130,246,.2)}
.ci-modal-icon-success{background:rgba(34,197,94,.1);border:1px solid rgba(34,197,94,.2)}
.ci-modal-icon-gold{background:rgba(232,160,32,.1);border:1px solid rgba(232,160,32,.2)}
...
.ci-modal-footer{display:flex;gap:10px;justify-content:center;margin-top:20px;flex-wrap:wrap}
...
.ci-alert-info{background:rgba(59,130,246,.08);border:1px solid rgba(59,130,246,.2);color:#93c5fd}
.ci-alert-success{background:rgba(34,197,94,.08);border:1px solid rgba(34,197,94,.2);color:#86efac}
.ci-alert-gold{background:rgba(232,160,32,.08);border:1px solid rgba(232,160,32,.2);color:#fde68a}
...
.ci-btn-primary{background:var(--accent);color:#0a0d14;border-color:var(--accent)}
.ci-btn-primary:hover{opacity:.9}
.ci-btn-gold-outline{background:transparent;border-color:rgba(232,160,32,.4);color:var(--accent)}
.ci-btn-gold-outline:hover{background:rgba(232,160,32,.08)}

/* ── ACCOUNT MODAL CONTENT ── */
.am-option{display:flex;align-items:center;gap:14px;padding:14px 16px;border-radius:12px;cursor:pointer;transition:all .15s;border:1px solid transparent;width:100%;background:var(--surface2);margin-bottom:10px;text-align:left}
.am-option:hover{border-color:var(--accent);background:rgba(232,160,32,.04)}
.am-option.danger:hover{border-color:rgba(239,68,68,.4);background:rgba(239,68,68,.05)}
.am-icon{width:40px;height:40px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0}
.am-title{font-size:13px;font-weight:600;color:var(--text)}
.am-sub{font-size:11px;color:var(--text3);margin-top:2px}

/* ── TOAST ── */
.ci-toast{background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:12px 16px;font-size:13px;color:var(--text);display:flex;align-items:center;gap:10px;min-width:240px;max-width:360px;pointer-events:all;box-shadow:0 4px 20px rgba(0,0,0,.4);animation:ci-toast-in .25s ease;cursor:pointer}
...
.ci-toast-warning{border-color:rgba(251,191,36,.3)}

/* ── RESPONSIVE ── */
@media(max-width:900px){.ci-stats-grid{grid-template-columns:repeat(2,1fr)}.ci-hide-md{display:none}}
@media(max-width:600px){.ci-stats-grid{grid-template-columns:1fr}.ci-hide-sm{display:none}.ci-filters{flex-direction:column}.ci-filter-group{min-width:100%}}
</style>
...
(function() {
    ...
    function init() {
        ...
        s.value = filters.search; sec.value = filters.sector; srt.value = filters.sort;
        updateReset();

        s.addEventListener('input', () => { clearTimeout(debTimer); debTimer = setTimeout(applyFilters, 380); });
        ...
        document.getElementById('pagination-container').addEventListener('click', e => {
            ...
            if (page) { filters.page = page; fetchData(); }
        });
    }
    ...
    function resetFilters() {
        filters = {search:'', sector:'', sort:'name', page:1};
