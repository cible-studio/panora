...
<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin-bottom:20px;">
    @foreach([
        ['label'=>'Total',         'key'=>'total',      'color'=>'var(--text)',   'icon'=>'📋', 'status'=>''],
        ['label'=>'Non confirmées','key'=>'en_attente', 'color'=>'var(--accent)', 'icon'=>'⏳', 'status'=>'en_attente'],
        ['label'=>'Confirmées',    'key'=>'confirme',   'color'=>'var(--green)',  'icon'=>'✅', 'status'=>'confirme'],
        ['label'=>'Refusées',      'key'=>'refuse',     'color'=>'var(--red)',    'icon'=>'❌', 'status'=>'refuse'],
        ['label'=>'Annulées',      'key'=>'annule',     'color'=>'var(--text2)',  'icon'=>'🚫', 'status'=>'annule'],
    ] as $s)
    <a href="{{ route('admin.reservations.index', array_merge(request()->except(['status','page']), $s['status'] ? ['status'=>$s['status']] : [])) }}"
       style="text-decoration:none;">
        <div class="stat-card"
             style="{{ request('status') === $s['status'] && $s['status'] !== ''
            ...
            @if($s['key'] === 'en_attente' && ($newCount ?? 0) > 0)
        ...
        <div style="display:flex;align-items:center;gap:10px;">
            ...
            @if(($newCount ?? 0) > 0)
                <span style="font-size:12px;font-weight:600;color:var(--accent);
                             ...
                             border-radius:20px;border:1px solid rgba(232,160,32,.3);">
                    ✦ {{ $newCount }} nouvelle(s) depuis votre dernière visite
                </span>
            @endif
        </div>
    </div>
    ...
    <div class="table-wrap">
        <table>
            ...
            <tbody>
                ...
                @php
                    $isNew         = isset($lastSeenAt) && $res->created_at > $lastSeenAt;
                ...
                <tr style="{{ $isNew ? 'background:rgba(232,160,32,.04);' : '' }}">
                    ...
                    <td style="font-weight:600;color:var(--accent);font-size:13px;
                               white-space:nowrap;">
                        {{ number_format($res->total_amount, 0, ',', ' ') }} FCFA
                    </td>
                    ...
                    <td>
                        ...
                        @endphp
                        <span class="badge {{ $badgeMap[$res->status->value] ?? 'badge-gray' }}">
                            {{ $res->status->label() }}
                        </span>
                        @if($clientDeleted)
                    ...
                    <td>
                        <div style="display:flex;gap:4px;align-items:center;">
                            ...
                            {{-- Annuler : en_attente ou confirme + client actif + droits --}}
                            @if($canAnnuler)
        ...
        <div class="modal-body">
            <div style="text-align:center;margin-bottom:20px;">
                ...
                <div style="font-size:13px;color:var(--text2);">
                    Réservation de
                    <strong id="annuler-client" style="color:var(--text);"></strong>
                </div>
            </div>
            ...
            <div style="background:var(--surface2);border:1px solid var(--border2);
                ...
                <div style="display:flex;flex-direction:column;gap:8px;color:var(--text2);">
                    <div style="display:flex;gap:8px;">
                        ...
                        <span>
                            Les <strong id="annuler-panels" style="color:var(--text);"></strong>
                            panneau(x) seront <strong>libérés immédiatement</strong>.
                        </span>
                    </div>
                    <div style="display:flex;gap:8px;">
                        <span style="color:var(--green);flex-shrink:0;">✓</span>
                        <span>L'historique de la réservation sera <strong>conservé</strong>.</span>
                    </div>
                </div>
            </div>
            ...
            <div style="background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.2);
                ...
                <span>⚠️</span>
                <span>Cette action est <strong>irréversible</strong>.
                    Une réservation annulée ne peut pas être réactivée.</span>
            </div>
        </div>
    ...
    <div class="modal" style="width:420px;" onclick="event.stopPropagation()">
        ...
        <div class="modal-body" style="text-align:center;padding:28px 22px;">
            ...
            <div style="font-size:13px;color:var(--text2);line-height:1.6;margin-bottom:14px;">
                Cette réservation sera <strong style="color:var(--red);">supprimée
                définitivement</strong>. Cette action ne peut pas être annulée.
            </div>
            <div style="background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.2);
...
// Marquer comme "vu" après 2s — appel silencieux
setTimeout(() => {
