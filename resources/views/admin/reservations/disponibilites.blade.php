        ...
        <div class="flex items-center justify-between mb-4 flex-wrap gap-3">
            <div class="flex items-center gap-2 flex-wrap">
                ...
                <button id="btn-select-all" onclick="DISPO.selectAll()"
                    class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold bg-[var(--surface)] border border-[var(--accent)]/40 rounded-xl text-[var(--accent)] hover:bg-[var(--accent)]/10 transition-all">
                    ☑ Tout sélectionner
                </button>
            </div>
            ...
            (function() {
                ...
                const STATUS_CFG = {
                    ...
                    maintenance: {
                        l: 'Maintenance',
                        c: '#6b7280',
                        b: 'rgba(107,114,128,.08)',
                        bd: 'rgba(107,114,128,.3)'
                    },
                    a_verifier: {
                        l: 'À vérifier',
                        c: '#94a3b8',
                        b: 'rgba(148,163,184,.08)',
                        bd: 'rgba(148,163,184,.3)'
                    },
                };
                ...
                window.DISPO = {
                    ...
                    reset() {
                        ...
                        S.page = 1;
                        ['f-dimensions', 'f-is_lit'].forEach(id => {
                            const el = _el(id);
                            if (el) el.value = '';
                        });
                        const s = _el('f-statut');
                        if (s) s.value = 'tous';
                        const r = _el('f-source');
                        if (r) r.value = 'all';
                        _el('f-du').value = '';
                        _el('f-au').value = '';
                        _el('f-search').value = '';
                    ...
                    setView(mode) {
                        ...
                        const grid = _el('panels-grid'),
                            list = _el('panels-list'),
                            btnG = _el('btn-view-grid'),
                            btnL = _el('btn-view-list');
                        if (!grid || !list) return;
                        const on =
                            'px-3 py-1.5 rounded-lg text-xs font-bold transition-all bg-[var(--accent)] text-white';
                        const off =
                            'px-3 py-1.5 rounded-lg text-xs font-bold transition-all text-[var(--text3)] hover:text-[var(--text)]';
                        if (mode === 'grid') {
                    ...
                    nextPage() {
                        if (S.page < S.pages) {
                            S.page++;
                            this._fetch();
                            _el('panels-grid')?.scrollIntoView({
                                behavior: 'smooth',
                                block: 'start'
                            });
                        }
                    ...
                    // ── SÉLECTION ─────────────────────────────────────────
                    toggle(id, rate, source, startDate) {
                        id = String(id);
                        ...
                        if (idx === -1) {
                            ...
                            if (startDate) S.sel.start_dates[id] = startDate;
                        } else {
                    ...
                    selectAll() {
                        const panels = S._lastPanels.filter(p => p.is_selectable);
                    ...
                    openConfirmModal() {
                        ...
                        _el('hidden-panels').innerHTML = S.sel.ids
                            ...
                            .join('');
                        Object.entries(S.sel.start_dates || {}).forEach(([id, date]) => {
                            const inp = document.createElement('input');
                            inp.type = 'hidden';
                            inp.name = `panel_start_dates[${id}]`;
                            inp.value = date;
                        ...
                        const futureCount = Object.keys(S.sel.start_dates || {}).length;
                        _el('modal-summary').textContent = `${S.sel.ids.length} panneau(x) sélectionné(s)${futureCount > 0 ? ` (dont ${futureCount} à démarrage décalé)` : ''}`;
                    ...
                    calcEstimate() {
                        ...
                        const total = S.sel.ids.reduce((s, id) => {
                            const panelDu = (S.sel.start_dates || {})[id] || du;
                            return s + (S.sel.rates[id] || 0) * _months(panelDu, au);
                        }, 0);
                        ...
                        if (this._customAmount !== null && this._customAmount > 0) {
                            ...
                            const hasMixed = Object.keys(S.sel.start_dates || {}).length > 0;
                            monthsEl.textContent = hasMixed ? `(${months} mois, durées variables)` : `(${months} mois)`;
                        }
                ...
                ${p.zone_description ? `<div style="font-size:10px;color:var(--text3);font-weight:700;text-transform:uppercase;margin-bottom:4px">Zone</div><div style="background:var(--surface2);border-radius:10px;padding:12px;font-size:12px;color:var(--text2)">${p.zone_description}</div>` : ''}`;
                    ...
                    async submitQuickClient(event) {
                        ...
                        const btn = _el('qc-submit'),
                            errBox = _el('quick-client-errors');
                        errBox.classList.add('hidden');
                        const name = _el('qc-name').value.trim();
                        if (!name) {
                            errBox.innerHTML =
                                '<div class="flex gap-2"><span>⚠️</span><span>Le nom est obligatoire.</span></div>';
                        ...
                        try {
                            const res = await fetch(
                                '{{ secure_url(route('admin.clients.quick-store', [], false)) }}', {
                                    method: 'POST',
                                    headers: {
                            ...
                            if (!res.ok) {
                                ...
                                errBox.innerHTML = messages.map(m =>
                                    `<div class="flex gap-2"><span>⚠️</span><span>${m}</span></div>`).join('');
                                errBox.classList.remove('hidden');
                                return;
                            }
                    ...
                    _cardHtml(p) {
                        ...
                        const releaseHtml = p.release_info ?
                            ...
                            '';
                        const isFutureSel = !p.is_selectable && !!p.is_future_selectable;
                        const isClickable = p.is_selectable || isFutureSel;
                        const tArgs = isFutureSel
                            ? `'${p.id}',${p.monthly_rate},'${p.source}','${p.selectable_from}'`
                            : `'${p.id}',${p.monthly_rate},'${p.source}'`;
                        const selBtn = p.is_selectable ?
                            `<button type="button" class="btn-sel" style="flex:1.2;font-size:11px;padding:6px 10px;border-radius:7px;background:${isSel?'var(--accent)':'var(--surface3)'};color:${isSel?'#fff':'var(--text)'};border:1px solid ${isSel?'transparent':'var(--border2)'};cursor:pointer;transition:all .15s;" onclick="event.stopPropagation();DISPO.toggle(${tArgs})">${isSel?'✓ Sélectionné':'+ Sélectionner'}</button>` :
                            isFutureSel ?
                            `<button type="button" class="btn-sel" style="flex:1.2;font-size:11px;padding:6px 10px;border-radius:7px;background:${isSel?'var(--accent)':'rgba(249,115,22,.08)'};color:${isSel?'#fff':'#f97316'};border:1px solid ${isSel?'transparent':'rgba(249,115,22,.4)'};cursor:pointer;transition:all .15s;" onclick="event.stopPropagation();DISPO.toggle(${tArgs})">${isSel?'✓ Sélectionné':'📅 Dispo le '+p.selectable_from_label}</button>` :
                        ...
                        return `<div class="panel-card${isClickable?' selectable':''}${isSel?' selected':''}" data-id="${p.id}" ${isClickable?`onclick="DISPO.toggle(${tArgs})"`:''}>` +
                            ...
                            ` ${isClickable?`<div style="position:absolute;top:10px;left:10px;z-index:2;"><input type="checkbox" class="card-chk" style="accent-color:var(--accent);width:16px;height:16px;cursor:pointer;" ${isSel?'checked':''} onclick="event.stopPropagation();DISPO.toggle(${tArgs})"></div>`:''}` +
                            `<div style="position:absolute;top:8px;right:8px;z-index:2;padding:4px 10px;border-radius:20px;font-size:10px;font-weight:700;background:${sc.c};color:white;text-transform:uppercase;letter-spacing:.5px;box-shadow:0 2px 8px rgba(0,0,0,.3);">${sc.l}</div>` +
                            `<div style="height:96px;flex-shrink:0;position:relative;overflow:hidden;${thumbSt}"><div style="position:absolute;inset:0;background:${p.photo_url?'linear-gradient(to bottom,rgba(0,0,0,.1),rgba(0,0,0,.65))':'rgba(0,0,0,.15)'}"></div><div style="position:absolute;bottom:8px;left:50%;transform:translateX(-50%);background:rgba(0,0,0,.75);border-radius:7px;padding:4px 14px;font-family:monospace;font-size:13px;font-weight:700;color:#fff;letter-spacing:1.5px;white-space:nowrap;backdrop-filter:blur(4px);">${p.reference}</div></div>` +
                            `<div style="padding:12px 14px;flex:1;display:flex;flex-direction:column;"><div style="font-size:10px;color:var(--text3);margin-bottom:2px;">${p.commune}${p.zone&&p.zone!=='—'?' · '+p.zone:''}</div><div style="font-weight:700;font-size:13px;color:var(--text);margin-bottom:8px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${p.name}">${p.name}</div><div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:6px;">${tags}</div>${p.zone_description?`<div style="font-size:11px;color:var(--text2);margin-bottom:6px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${p.zone_description}">📍 ${p.zone_description}</div>`:''}` +
                            `<div style="margin-top:auto;padding-top:8px;border-top:1px solid var(--border);"><div style="font-size:17px;font-weight:800;color:var(--accent);margin-bottom:6px;">${p.monthly_rate?Math.round(p.monthly_rate/1000).toLocaleString('fr-FR')+'K <span style="font-size:11px;font-weight:400;color:var(--text3)">FCFA/mois</span>':'<span style="font-size:13px;color:var(--text3)">Tarif non défini</span>'}</div>${releaseHtml}<div style="display:flex;gap:6px;margin-top:8px;"><button type="button" style="flex:none;font-size:10px;padding:6px 10px;border-radius:7px;background:var(--surface);border:1px solid var(--border);color:var(--text2);cursor:pointer;" onclick="event.stopPropagation();DISPO.openFiche(JSON.parse(decodeURIComponent(this.dataset.p)))" data-p="${safeP}">📋 Fiche</button>${selBtn}</div></div></div></div>`;
                    },
                    ...
                    _renderList(panels) {
                        ...
                        panels.forEach(p => {
                            ...
                            else if (isFutureSel) tr.onclick = () => DISPO.toggle(p.id, p.monthly_rate, p.source, p.selectable_from);
                            const safeP = encodeURIComponent(JSON.stringify(p));
                            const chkHtml = isClickable
                                ? `<input type="checkbox" class="card-chk" style="accent-color:var(--accent);width:15px;height:15px;cursor:pointer;" ${isSel?'checked':''} onclick="event.stopPropagation();DISPO.toggle(${tArgs})">`
                                : `<span style="font-size:12px;opacity:.4;">🔒</span>`;
                            const futureLabel = isFutureSel ? `<div style="font-size:9px;color:#f97316;margin-top:2px;">📅 Dispo le ${p.selectable_from_label}</div>` : '';
                            tr.innerHTML =
                                `<td style="padding:10px 8px;width:36px;text-align:center;">${chkHtml}</td><td style="padding:10px 8px;"><span style="font-family:monospace;font-weight:700;font-size:12px;padding:3px 8px;border-radius:6px;background:${sc.b};color:${sc.c}">${p.reference}</span>${p.source==='external'?`<span style="display:block;font-size:9px;color:#60a5fa;margin-top:2px;">🤝 ${p.agency_name}</span>`:''}</td><td style="padding:10px 8px;"><div style="font-weight:600;font-size:13px;color:var(--text);">${p.name}</div><div style="font-size:11px;color:var(--text3);">${p.commune}${p.zone&&p.zone!=='—'?' · '+p.zone:''}</div></td><td style="padding:10px 8px;font-size:12px;color:var(--text2);">${p.format||'—'}</td><td style="padding:10px 8px;font-size:12px;color:var(--text2);">${p.dimensions||'—'}${p.is_lit?' 💡':''}</td><td style="padding:10px 8px;"><div style="font-weight:700;color:var(--accent);font-size:13px;">${p.monthly_rate?Math.round(p.monthly_rate/1000).toLocaleString('fr-FR')+'K':'—'} <span style="font-size:10px;font-weight:400;color:var(--text3)">FCFA</span></div></td><td style="padding:10px 8px;"><span style="font-size:10px;font-weight:700;padding:3px 8px;border-radius:20px;background:${sc.b};color:${sc.c};border:1px solid ${sc.bd}">${sc.l}</span>${p.release_info?`<div style="font-size:10px;color:var(--text3);margin-top:3px;">📅 ${p.release_info.label}</div>`:''}${futureLabel}</td><td style="padding:10px 8px;"><button type="button" style="font-size:10px;padding:5px 10px;border-radius:6px;background:var(--surface2);border:1px solid var(--border2);color:var(--text2);cursor:pointer;" onclick="event.stopPropagation();DISPO.openFiche(JSON.parse(decodeURIComponent(this.dataset.p)))" data-p="${safeP}">📋 Fiche</button></td>`;
                            frag.appendChild(tr);
                        });
                        tbody.innerHTML = '';
                    ...
                    _renderPagination(stats) {
                        ...
                        const from = (S.page - 1) * S.perPage + 1,
                            to = Math.min(S.page * S.perPage, stats.total);
                    ...
                    _syncSelBar() {
                        ...
                        if (btnSA) {
                            const selectablePanels = S._lastPanels.filter(p => p.is_selectable);
                            const allSel = selectablePanels.length > 0 && selectablePanels.every(p => S.sel.ids.includes(String(p.id)));
                    ...
                    _renderTags() {
                        ...
                        const addMS = (ids, key, data) => ids.forEach(id => {
                            ...
                            if (it) tags.push({
                                l: it.name,
                                rm: () => {
                                    const i = S.f[key].indexOf(id);
                                    if (i > -1) S.f[key].splice(i, 1);
                                    S.page = 1;
                                    _syncMs(key);
                                    this._fetch();
                                    this._syncUI();
                        ...
                        if (f.dimensions) tags.push({
                            ...
                            rm: () => {
                                S.f.dimensions = '';
                                _el('f-dimensions').value = '';
                                S.page = 1;
                                this._fetch();
                                this._syncUI();
                            }
                        });
                        if (f.is_lit === '1') tags.push({
                            l: '💡 Éclairé',
                            rm: () => {
                                S.f.is_lit = '';
                                _el('f-is_lit').value = '';
                        ...
                        if (f.statut !== 'tous') tags.push({
                            l: 'Statut: ' + f.statut,
                            rm: () => {
                                S.f.statut = 'tous';
                                _el('f-statut').value = 'tous';
                                S.page = 1;
                                this._fetch();
                                this._syncUI();
                            }
                ...
                function buildMs(wrapper) {
                    const key = wrapper.dataset.key,
                        ph = wrapper.dataset.placeholder || 'Sélectionner',
                        data = MS_DATA[key] || [];
                    ...
                    foot.innerHTML =
                        `<span id="ms-foot-${key}">0 sélectionné(s)</span><div><button type="button" onclick="__msAll('${key}')">Tout</button><button type="button" onclick="__msClear('${key}')">Aucun</button></div>`;
                    ...
                    function updateTrigger() {
                        ...
                        if (!sel.length) {
                            ...
                            inner.innerHTML = sel.map(id => {
                                const it = data.find(x => x.id === id || x.id === parseInt(id));
                                return it ?
                                    `<span class="ms-chip">${it.name}<button type="button" onclick="event.preventDefault();event.stopPropagation();__msRemove('${key}',${id})" title="Retirer">✕</button></span>` :
                                    '';
                            }).join('');
                        }
                    ...
                    si.addEventListener('input', () => {
                        clearTimeout(stimer);
                        stimer = setTimeout(() => render(si.value), 150);
                    });
                ...
                function _syncMs(key) {
                    MS[key]?.updateTrigger();
                }

                function _closeAllMs() {
                    Object.values(MS).forEach(m => {
                        m.drop.style.display = 'none';
