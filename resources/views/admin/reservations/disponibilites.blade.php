    ...
    <div class="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-[var(--border)]">
        <div class="flex flex-wrap items-center gap-3">
            ...
            <div class="flex items-center gap-2 bg-[var(--surface2)] px-3 py-1.5 rounded-xl border border-[var(--border2)]">
                <input type="date" id="f-du" class="bg-transparent border-none text-sm text-[var(--text)] focus:outline-none" onchange="DISPO.onDateChange('du', this.value)">
                <span class="text-[var(--text3)] text-xs">→</span>
                <input type="date" id="f-au" class="bg-transparent border-none text-sm text-[var(--text)] focus:outline-none" onchange="DISPO.onDateChange('au', this.value)">
            </div>
...
<div class="flex items-center justify-between mb-4 flex-wrap gap-3">
    ...
    <div class="flex gap-2 flex-wrap">
        ...
        <button onclick="DISPO.exportPdf('liste')"
...
{{-- ══ BARRE SÉLECTION ══ --}}
<div id="sel-bar" style="display:none;position:fixed;bottom:0;left:235px;right:0;z-index:300;background:var(--surface);border-top:2px solid var(--accent);padding:12px 24px;box-shadow:0 -8px 32px rgba(0,0,0,.2)">
    <div class="flex items-center justify-between flex-wrap gap-3">
        <div class="flex items-center gap-4">
            ...
            <div id="sel-ext-badge" class="hidden px-2 py-0.5 text-xs text-blue-500 border border-blue-500/30 bg-blue-500/10 rounded-lg">
...
<div id="modal-quick-client"
    ...
    <div class="bg-[var(--surface)] border border-[var(--border2)] rounded-2xl w-full max-w-md shadow-2xl"
        ...
        <form id="form-quick-client" onsubmit="DISPO.submitQuickClient(event)" class="p-5 space-y-4">
            ...
            <div id="quick-client-errors" class="hidden bg-red-500/10 border border-red-500/30 rounded-xl p-3 text-sm text-red-500 space-y-1"></div>
...
<div id="modal-error" class="fixed inset-0 z-[10000] bg-black/70 backdrop-blur-sm items-center justify-center p-4"
    ...
    <div class="bg-[var(--surface)] border border-red-500/40 rounded-2xl w-full max-w-md shadow-2xl"
        ...
        <div class="p-5"><div id="error-body" class="text-sm text-[var(--text2)] space-y-2"></div></div>
...
#modal-confirm ~ .select2-container .select2-dropdown { z-index: 10001 !important; }

.filter-label  { font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.5px;color:var(--text3); }
.filter-select { height:40px;padding:0 12px;background:var(--surface2);border:1px solid var(--border2);border-radius:10px;font-size:13px;color:var(--text);cursor:pointer;transition:border-color .2s;width:100%; }
.filter-select:hover,.filter-select:focus { border-color:var(--accent);outline:none; }
.ms-badge    { background:var(--accent);color:#fff;border-radius:9999px;padding:1px 8px;font-size:10px;font-weight:700; }
.stat-pill   { display:inline-flex;align-items:center;gap:4px;padding:3px 10px;background:var(--surface2);border:1px solid var(--border2);border-radius:9999px;font-size:12px;color:var(--text2); }
.modal-input { background:var(--surface2);border:1px solid var(--border2);border-radius:10px;padding:9px 12px;font-size:13px;color:var(--text);transition:border-color .2s;width:100%; }
.modal-input:focus { border-color:var(--accent);outline:none;box-shadow:0 0 0 2px var(--accent-dim); }
.list-th { padding:10px 8px;text-align:left;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:var(--text3);white-space:nowrap; }
.tag { background:var(--surface3);color:var(--text2);font-size:10px;padding:2px 6px;border-radius:4px; }
.ms-wrapper { position:relative; }
.ms-btn { width:100%;min-height:40px;padding:6px 30px 6px 12px;background:var(--surface2);border:1px solid var(--border2);border-radius:10px;font-size:13px;color:var(--text);cursor:pointer;text-align:left;display:flex;align-items:center;flex-wrap:wrap;gap:4px;position:relative;transition:border-color .2s; }
.ms-btn:hover,.ms-btn.open { border-color:var(--accent); }
.ms-btn::after  { content:"▾";position:absolute;right:10px;top:50%;transform:translateY(-50%);color:var(--text3);font-size:11px;pointer-events:none; }
.ms-btn.open::after { content:"▴"; }
.ms-placeholder { color:var(--text3);font-size:12px; }
.ms-chip { background:var(--accent-dim);color:var(--accent);border-radius:6px;padding:2px 6px;font-size:11px;display:inline-flex;align-items:center;gap:3px; }
.ms-chip button { background:none;border:none;color:var(--accent);cursor:pointer;opacity:.6;font-size:11px;padding:0; }
.ms-chip button:hover { opacity:1; }
.ms-drop { position:absolute;top:calc(100% + 4px);left:0;right:0;z-index:500;background:var(--surface2);border:1px solid var(--border2);border-radius:12px;box-shadow:0 8px 24px rgba(0,0,0,.15);max-height:280px;display:flex;flex-direction:column;overflow:hidden; }
.ms-search { padding:8px;border-bottom:1px solid var(--border); }
.ms-search input { width:100%;height:32px;padding:0 10px;background:var(--surface);border:1px solid var(--border2);border-radius:8px;font-size:12px;color:var(--text);outline:none; }
.ms-search input:focus { border-color:var(--accent); }
.ms-list { overflow-y:auto;flex:1; }
.ms-opt { padding:9px 12px;font-size:13px;cursor:pointer;display:flex;align-items:center;gap:8px;color:var(--text2);border-bottom:1px solid var(--border);transition:all .15s; }
...
.ms-foot { padding:6px 12px;border-top:1px solid var(--border);background:var(--surface3);display:flex;justify-content:space-between;font-size:11px;color:var(--text3); }
.ms-foot button { background:none;border:none;color:var(--accent);cursor:pointer;font-size:11px; }
.panel-card { background:var(--surface2);border-radius:14px;overflow:hidden;border:2px solid var(--border);transition:transform .15s,box-shadow .15s,border-color .15s;position:relative;display:flex;flex-direction:column; }
.panel-card:hover { transform:translateY(-3px);box-shadow:0 8px 24px rgba(0,0,0,.12); }
.panel-card.selected { border-color:var(--accent) !important;box-shadow:0 0 0 3px var(--accent-dim) !important; }
...
window.DISPO = {
    ...
    setView(mode){
        ...
        if(mode==='grid'){grid.style.display='grid';list.style.display='none';btnG.className=on;btnL.className=off;}
        else{grid.style.display='none';list.style.display='block';btnG.className=off;btnL.className=on;if(S._lastPanels.length>0)this._renderList(S._lastPanels);}
    },
    exportPdf(type){
    ...
    exportSelPdf(type){
        ...
        if(ids.length===0){alert('Aucun panneau interne sélectionné.');return;}
        const fId=type==='images'?'form-pdf-images':'form-pdf-liste';
        const iId=type==='images'?'pdf-images-inputs':'pdf-liste-inputs';
        const sId=type==='images'?'pdf-start':'pdf-liste-start';
        const eId=type==='images'?'pdf-end':'pdf-liste-end';
    ...
    toggle(id,rate,source){
        ...
        if(card){card.classList.toggle('selected',sel);const btn=card.querySelector('.btn-sel');if(btn){btn.textContent=sel?'✓ Sélectionné':'+ Sélectionner';btn.style.background=sel?'var(--accent)':'var(--surface3)';btn.style.color=sel?'#fff':'var(--text)';}const chk=card.querySelector('.card-chk');if(chk)chk.checked=sel;}
    ...
    openConfirmModal(){
        ...
        _el('modal-errors').classList.add('hidden');_el('modal-date-err').classList.add('hidden');_el('modal-client-err').classList.add('hidden');
    ...
    submitForm(){
        const du=_el('modal-du').value,au=_el('modal-au').value,client=$('#modal-client-select').val(),errors=[];
    ...
    showError(msgs){_el('error-body').innerHTML=(Array.isArray(msgs)?msgs:[msgs]).map(m=>`<div class="flex gap-2 items-start"><span class="text-red-500">•</span><span>${m}</span></div>`).join('');_show('modal-error');},
    ...
    openQuickClientModal(){
        ...
        _el('qc-submit-txt').textContent='Créer le client';_el('qc-submit').disabled=false;
        _show('modal-quick-client');setTimeout(()=>_el('qc-name')?.focus(),100);
    },
    closeQuickClientModal(){_hide('modal-quick-client');},
    async submitQuickClient(event){
        event.preventDefault();
        const btn=_el('qc-submit'),errBox=_el('quick-client-errors');errBox.classList.add('hidden');
        ...
        }catch(err){errBox.innerHTML=`<div class="flex gap-2"><span>⚠️</span><span>Erreur réseau : ${err.message}</span></div>`;errBox.classList.remove('hidden');}
        finally{_el('qc-submit-icon').textContent='🏢';_el('qc-submit-txt').textContent='Créer le client';btn.disabled=false;}
    },
    ...
    async _doFetch(){
        ...
        S.f.commune_ids.forEach(id=>p.append('commune_ids[]',id));S.f.zone_ids.forEach(id=>p.append('zone_ids[]',id));S.f.format_ids.forEach(id=>p.append('format_ids[]',id));S.f.agency_ids.forEach(id=>p.append('agency_ids[]',id));
        if(S.f.dimensions)p.set('dimensions',S.f.dimensions);if(S.f.is_lit!=='')p.set('is_lit',S.f.is_lit);if(S.f.statut!=='tous')p.set('statut',S.f.statut);
    ...
    _cardHtml(p){
        ...
        const tags=[p.format?`<span class="tag">${p.format}</span>`:'',p.dimensions?`<span class="tag">${p.dimensions}</span>`:'',p.is_lit?`<span class="tag" style="color:var(--accent)">💡</span>`:''].filter(Boolean).join('');
        const releaseHtml=p.release_info?`<div style="margin-top:4px;padding:4px 8px;border-radius:6px;font-size:10px;background:rgba(226,6,19,.06);border:1px solid rgba(226,6,19,.15);"><span style="color:${p.release_info.color==='green'?'#22c55e':p.release_info.color==='orange'?'var(--accent)':'var(--text3)'}">📅 ${p.release_info.label}</span></div>`:'';
        const selBtn=p.is_selectable?`<button type="button" class="btn-sel" style="flex:1.2;font-size:11px;padding:6px 10px;border-radius:7px;background:${isSel?'var(--accent)':'var(--surface3)'};color:${isSel?'#fff':'var(--text)'};border:1px solid ${isSel?'transparent':'var(--border2)'};cursor:pointer;transition:all .15s;" onclick="event.stopPropagation();DISPO.toggle('${p.id}',${p.monthly_rate},'${p.source}')">${isSel?'✓ Sélectionné':'+ Sélectionner'}</button>`:`<div style="flex:1.2;padding:6px 10px;background:var(--surface3);border-radius:7px;font-size:11px;color:var(--text3);text-align:center;border:1px solid var(--border);">${sc.l}</div>`;
