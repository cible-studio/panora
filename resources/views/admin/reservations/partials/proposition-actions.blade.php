...
<div class="proposition-block" style="
...
">
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:8px">
    ...
    @if($reservation->proposition_token)
      @if($reservation->proposition_expires_at?->isPast())
        <span style="background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.25);color:#fca5a5;border-radius:20px;padding:3px 12px;font-size:11px;font-weight:600">
          ⏰ Expirée
        </span>
      @elseif($reservation->proposition_viewed_at)
        <span style="background:rgba(34,197,94,0.1);border:1px solid rgba(34,197,94,0.25);color:#86efac;border-radius:20px;padding:3px 12px;font-size:11px;font-weight:600">
          👁️ Vue par le client
        </span>
      @else
        <span style="background:rgba(59,130,246,0.1);border:1px solid rgba(59,130,246,0.25);color:#93c5fd;border-radius:20px;padding:3px 12px;font-size:11px;font-weight:600">
          📤 Envoyée · En attente
        </span>
      @endif
    @endif
  ...
  <div style="background:rgba(0,0,0,0.15);border-radius:8px;padding:14px 16px;margin-bottom:16px;font-size:12px;color:#94a3b8;display:flex;flex-wrap:wrap;gap:16px">
    ...
    {{-- Lien client copiable --}}
    @if($reservation->lien_proposition)
    <div style="width:100%;margin-top:4px">
      <span style="color:#64748b">Lien client</span>
      <div style="display:flex;align-items:center;gap:8px;margin-top:4px">
        <input
          type="text"
          value="{{ $reservation->lien_proposition }}"
          readonly
        ...
        <a
          href="{{ $reservation->lien_proposition }}"
          target="_blank"
  ...
  <div style="background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.2);border-radius:8px;padding:10px 14px;margin-bottom:14px;font-size:12px;color:#fca5a5;display:flex;align-items:center;gap:8px">
    ⚠️ Le client <strong>{{ $reservation->client?->name }}</strong> n'a pas d'email renseigné.
  ...
  <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">
    ...
    {{-- Bouton principal : Envoyer / Renvoyer --}}
    @if(!empty($reservation->client?->email))
    <form
      method="POST"
      action="{{ route('admin.reservations.proposition.envoyer', $reservation) }}"
      onsubmit="return confirm('{{ $reservation->proposition_sent_at ? 'Renvoyer la proposition à ' . $reservation->client?->email . ' ?' : 'Envoyer la proposition à ' . $reservation->client?->email . ' ?' }}')"
    >
      @csrf
      <button
        type="submit"
        style="background:#e8a020;color:#0b0e17;font-weight:700;font-size:13px;padding:9px 20px;border-radius:8px;border:none;cursor:pointer;display:flex;align-items:center;gap:6px"
      >
        📧 {{ $reservation->proposition_sent_at ? 'Renvoyer la proposition' : 'Envoyer la proposition' }}
      </button>
    </form>
    @endif

    {{-- Réinitialiser (si proposition en cours) --}}
    @if($reservation->proposition_token)
    <form
      method="POST"
      action="{{ route('admin.reservations.proposition.reinitialiser', $reservation) }}"
      onsubmit="return confirm('Réinitialiser la proposition ? Le lien client ne fonctionnera plus.')"
    >
      @csrf
      <button
        type="submit"
  ...
  try {
    navigator.clipboard.writeText(input.value).then(() => {
      ...
      setTimeout(() => {
        ...
        btn.style.background = 'rgba(232,160,32,0.1)';
        btn.style.borderColor = 'rgba(232,160,32,0.2)';
