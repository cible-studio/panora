<x-admin-layout>
            ...
            <div id="export-dropdown" class="hidden"
                ...
                <form method="GET" action="{{ route('admin.panels.export.list') }}" target="_blank">
                    ...
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;cursor:pointer;">
                        ...
                        <label for="hide-status" style="font-size:13px;color:var(--text2);cursor:pointer;">
                            Masquer le statut
                        </label>
                    </div>
    ...
    <div class="stats-grid" style="grid-template-columns:repeat(5,1fr);">
        <a href="{{ route('admin.panels.index', ['source' => 'all']) }}" class="stat-card"
        ...
        <a href="{{ route('admin.panels.index', ['source' => 'cible', 'status' => 'libre']) }}" class="stat-card"
            style="text-decoration:none;cursor:pointer;
              {{ request('status') === 'libre' ? 'border-color:var(--green);' : '' }}">
            <div class="stat-label">Libres</div>
            <div class="stat-value" style="color:var(--green);">{{ $panneauxLibres }}</div>
        </a>
        <a href="{{ route('admin.panels.index', ['source' => 'cible', 'status' => 'occupe']) }}" class="stat-card"
            style="text-decoration:none;cursor:pointer;
        ...
        <a href="{{ route('admin.panels.index', ['source' => 'cible', 'status' => 'maintenance']) }}" class="stat-card"
            style="text-decoration:none;cursor:pointer;
              {{ request('status') === 'maintenance' ? 'border-color:var(--red);' : '' }}">
            <div class="stat-label">Maintenance</div>
            <div class="stat-value" style="color:var(--red);">{{ $enMaintenance }}</div>
        </a>
        <a href="{{ route('admin.panels.index', ['source' => 'externe']) }}" class="stat-card"
            style="text-decoration:none;cursor:pointer;
                ...
                <div class="filter-bar">
                    ...
                    <div class="filter-group">
                        ...
                        <select name="status" class="filter-select" onchange="this.form.submit()">
                            ...
                            <option value="libre" {{ request('status') === 'libre' ? 'selected' : '' }}>Libre
                            </option>
                            <option value="occupe" {{ request('status') === 'occupe' ? 'selected' : '' }}>Occupé
                            </option>
                            <option value="option" {{ request('status') === 'option' ? 'selected' : '' }}>Option
                            </option>
                            <option value="confirme" {{ request('status') === 'confirme' ? 'selected' : '' }}>
                                Confirmé</option>
                ...
                <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @if ($panel->format->surface)
                                        <div style="font-size:11px;color:var(--text3);">
                                            {{ $panel->format->surface }}m²</div>
                                    @endif
                                </td>
                                <td style="text-align:center;">
                                    <span
                                        style="font-weight:700;color:var(--text2);">{{ $panel->nombre_faces ?? 1 }}</span>
                                </td>
                                <td>
                                    @if ($panel->quartier)
                    ...
                    @if (($source ?? 'all') !== 'cible' && $externalPanels->isNotEmpty() && !request('client_id'))
                        ...
                        @if (($source ?? 'all') === 'all' && $panels->isNotEmpty())
                            <tr>
                                <td colspan="11"
                                    style="padding:8px 12px;background:rgba(168,85,247,0.06);
                        border-top:2px solid rgba(168,85,247,0.3);border-bottom:1px solid rgba(168,85,247,0.2);">
                                    <span
                                        style="font-size:11px;font-weight:700;color:var(--purple);
                            ...
                            <tr style="background:rgba(168,85,247,0.02);">
                                ...
                                <td>
                                    ...
                                    @if ($ext->format?->surface)
                                        <div style="font-size:11px;color:var(--text3);">{{ $ext->format->surface }}m²
                                        </div>
                                    @endif
                                </td>
                                <td style="text-align:center;">
                                    <span
                                        style="font-weight:700;color:var(--text2);">{{ $ext->nombre_faces ?? 1 }}</span>
                                </td>
                                <td>
                                    @if ($ext->orientation)
