...
@php
$kpis = [
    ['label'=>'Non lues',      'val'=>$totalNonLues, 'color'=>'#e8a020', 'icon'=>'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>'],
    ['label'=>'Danger',        'val'=>$totalDanger,  'color'=>'#ef4444', 'icon'=>'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>'],
    ['label'=>'Avertissements','val'=>$totalWarning, 'color'=>'#f97316', 'icon'=>'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>'],
    ['label'=>'Informations',  'val'=>$totalInfo,    'color'=>'#3b82f6', 'icon'=>'<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>'],
];
@endphp
@foreach($kpis as $k)
...
<div style="background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:14px 18px;margin-bottom:16px">
    <form method="GET" action="{{ route('admin.alerts.index') }}" style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap">
        <div>
        ...
        <div style="display:flex;align-items:center;gap:7px;align-self:flex-end;height:40px">
            ...
            <label for="chk-non-lues" style="font-size:12px;color:var(--text2);cursor:pointer;font-weight:500">Non lues seulement</label>
        </div>
        ...
        <div id="alert-{{ $alerte->id }}"
            ...
            <div style="flex:1;min-width:0">
                <div style="display:flex;align-items:center;gap:7px;flex-wrap:wrap;margin-bottom:5px">
                    ...
                    {{-- Badge module --}}
                    <span style="padding:1px 8px;border-radius:20px;font-size:9px;font-weight:700;background:{{ $typeCfg['bg'] }};color:{{ $typeCfg['c'] }}">
                        {{ $typeCfg['label'] }}
                    </span>
                </div>
                ...
                <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
                    ...
                    @if($typeCfg['route'])
                    <a href="{{ route($typeCfg['route']) }}" style="font-size:11px;color:{{ $typeCfg['c'] }};text-decoration:none;font-weight:600">
                        Voir le module →
                    </a>
                    @endif
                    ...
                    @endphp
                    @if($directUrl)
                    <a href="{{ $directUrl }}" style="font-size:11px;color:var(--accent);text-decoration:none;font-weight:600;padding:2px 8px;background:rgba(232,160,32,.08);border:1px solid rgba(232,160,32,.2);border-radius:6px">
                        Accéder directement ↗
                    </a>
                    @endif
                    @endif
...
<style>
.albl { font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:var(--text3);display:block;margin-bottom:4px }
.asel { height:40px;padding:0 12px;background:var(--surface2);border:1px solid var(--border);border-radius:10px;font-size:13px;color:var(--text);cursor:pointer;outline:none }
</style>
...
<style>
@keyframes alertFade { to { opacity:0;transform:translateX(10px);max-height:0;padding:0 } }
</style>
@endpush
