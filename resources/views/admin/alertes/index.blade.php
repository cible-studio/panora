<x-admin-layout title="Alertes & Notifications">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px">
        ...
        @foreach ($kpis as $k)
            <div
                style="background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:16px 20px;border-left:4px solid {{ $k['color'] }}">
    ...
    @push('scripts')
        ...
        <script>
            // Cacher le badge immédiatement quand on arrive sur la page alertes
            document.addEventListener('DOMContentLoaded', function() {
                const badge = document.getElementById('alert-badge');
                if (badge) badge.style.display = 'none';
            });
            // Supprimer les alertes vues quand on quitte la page
            const alertIds = @json($alertes->pluck('id'));
