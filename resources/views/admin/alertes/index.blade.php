<x-admin-layout title="Alertes & Notifications">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px">
        ...
        @endphp
        @foreach ($kpis as $k)
            <div data-filter="{{ $k['filter'] }}"
                class="kpi-card filter-stat {{ request('niveau') === $k['filter'] ? 'active' : '' }}"
                ...
                <div
                    style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.6px;color:var(--text3);margin-top:4px">
                    {{ $k['label'] }}</div>
            </div>
        @endforeach
    </div>
        ...
        <div class="filter-bar" style="display:flex;gap:12px;align-items:flex-end;flex-wrap:wrap">
            ...
            <div class="filter-group">
                ...
                <div style="display:flex;align-items:center;gap:8px;height:38px">
                    ...
                    <label for="filter-non-lues"
                        style="font-size:12px;color:var(--text2);cursor:pointer;font-weight:500">Non lues
                        seulement</label>
                </div>
            </div>
    ...
    <div id="alerts-container"
        ...
        @include('admin.alertes.partials.alerts-list', [
            'alertes' => $alertes,
        ...
        <script>
            ...
            (function() {
                ...
                document.addEventListener('click', function(e) {
                    ...
                    fetch(`{{ route('admin.alerts.index') }}?${params}`, {
                            headers: {
                                'X-Requested-With': 'XMLHttpRequest',
                                'Accept': 'application/json'
                            }
            ...
            window.ALERTS = {
                ...
                showToast(message, type) {
                    ...
                    toast.style.cssText =
                        `position:fixed;bottom:24px;right:24px;z-index:99999;padding:12px 18px;background:var(--surface);border-left:3px solid ${colors[type] || '#22c55e'};border-radius:10px;font-size:13px;color:var(--text);box-shadow:0 8px 24px rgba(0,0,0,.25);animation:slideIn .3s ease;max-width:320px`;
            ...
            window.addEventListener('beforeunload', function() {
                ...
                navigator.sendBeacon('/admin/alerts/delete-seen', JSON.stringify({
                    ids: alertIds,
                    _token: '{{ csrf_token() }}'
                }));
            });
